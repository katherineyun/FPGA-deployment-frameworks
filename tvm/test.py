...
image = preprocess_input(image).transpose([0, 3, 1, 2])

# test prediction
keras_out = model.predict(image.transpose([0, 2, 3, 1]))
...
def inputs_func(iter):
    import os
    img_files = [os.path.join(quant_dir, f) for f in os.listdir(quant_dir) if f.endswith(('JPEG', 'jpg', 'png'))][:100]
    ...
    print("Number of quant images: ", len(imgs))
